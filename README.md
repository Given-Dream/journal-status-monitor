...
为了避免 GitHub 在下载 action 压缩包时偶发失败，当前 workflow 已尽量改为直接使用 runner 自带的 `git`、`python3` 和标准库完成 checkout、运行、提交数据和清理旧运行记录；运行记录只保留最近 1 条。这样也不会再触发 Node.js 20 action 弃用警告。
